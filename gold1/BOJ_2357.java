...
public class Main { // class name은 Main 이어야 함
	...
	static void init2(long[] a, long[] tree2, int node, int start, int end) { //최초 1회 N개의 노드를 가진 세그먼트 트리 생성
		if (start==end) tree2[node]=a[start];
		else {
			init2(a,tree2,2*node,start,(start+end)/2);
			init2(a,tree2,2*node+1,(start+end)/2+1,end);
	...
	static long query2(long[] tree2, int node, int start, int end, int left, int right) { //left~right 구간 합 구하기
		if (left>end || right<start) return -1; 
		if (left<=start && end<=right ) return tree2[node];
		
		long lmax=query2(tree2, node*2, start, (start+end)/2, left,right);
		long rmax=query2(tree2,node*2+1,(start+end)/2+1,end,left,right);
	...
	public static void main(String[] args) throws IOException { // 예외처리 꼭 해줘야 함
			...
			for (int i=0; i<m;i++) {
				...
				bw.write(Long.toString(query(tree, 1, 0, n-1,idx1,idx2))
						+" "+Long.toString(query2(tree2, 1, 0, n-1,idx1,idx2))
						+"\n");
			}
			bw.flush(); bw.close();
	   }
