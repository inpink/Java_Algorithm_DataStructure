...
public class Main { // class name은 Main 이어야 함
	...
	static int query(int[] a, int[] tree, int node, int start, int end, int left, int right) { 
		...
		else if (a[lmin]<=a[rmin]) return lmin; //index와 관련이 있는 값이니 여기까지 내려왔고, index값은 하나이니 둘 중 하나는 무조건 -1, 하나는 무조건 유효값일 수 밖에 없을텐데,, 그럼 이 코드도 필요없을텐데
		else return rmin;
	}
	...
	public static void main(String[] args) throws IOException { // 예외처리 꼭 해줘야 함
			...
			for (int i=0; i<m;i++) {
				...
				else bw.write(Long.toString(query(a, tree, 1, 0, n-1,Integer.parseInt(st1.nextToken())-1,Integer.parseInt(st1.nextToken())-1)+1)+"\n");
			}
			bw.flush(); bw.close();
	   }
}
